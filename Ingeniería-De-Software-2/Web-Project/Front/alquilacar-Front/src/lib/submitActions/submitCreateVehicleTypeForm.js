...
export async function submitCreateVehicleTypeForm(prevState, formData) {
    try {
        ...
        if (response.ok) {
            ...
            return {
                ...
                error: result.errors || {
                    generalError: ["Error al crear el tipo de vehiculo"],
                },
            };
        }
    } catch (error) {
        console.error("Error en submitCreateVehicleTypeForm:", error);
        return {
            error: {
