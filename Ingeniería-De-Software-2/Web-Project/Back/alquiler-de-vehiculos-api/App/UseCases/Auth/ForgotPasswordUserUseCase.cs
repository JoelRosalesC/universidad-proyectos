...
{
    public class ForgotPasswordUserUseCase
    {
        ...
        public ForgotPasswordUserUseCase(IVerifyCodeRepository verifyCodeRepository, IEmailQueueRepository emailQueueRepository, ConvertInEmailTemplateUseCase convertInEmailTemplateUseCase)
        {
        ...
        {
            string code = _verifyCodeRepository.Generate(email, VerifyCodeType.ResetPassword);
            ...
            EmailTemplate emailTemplate = await _convertInEmailTemplateUseCase.runAsync("Forgot Password", "10", code);
            await _emailQueueRepository.EnqueueAsync(email, "Cambiar contraseña", emailTemplate.Html);
        }
    }
}